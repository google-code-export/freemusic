...
class Dump(BaseRequestHandler):
	...
	def get_users(self):
		result = []
		for obj in model.SiteUser.all().fetch(1000):
			result.append({
				'user': obj.user.email(),
				'joined': obj.joined.strftime('%Y-%m-%d %H:%M:%S'),
				'weight': obj.weight,
	...
	def get_artists(self):
		result = []
		for obj in model.SiteArtist.all().fetch(1000):
			result.append({
				'id': obj.id,
				'name': obj.name,
				'sortname': obj.sortname,
				'albums': self.get_albums(obj),
			})
		return result

	def get_albums(self, artist):
		result = []
		for obj in model.SiteAlbum.gql('WHERE artist = :1', artist).fetch(1000):
			result.append({
				'id': obj.id,
				'name': obj.name,
				'text': obj.text,
				'release_date': obj.release_date.strftime('%Y-%m-%d %H:%M:%S'),
				'rating': obj.rating,
				'cover_small': obj.cover_small,
				'cover_large': obj.cover_large,
				'labels': obj.labels,
				'owner': obj.owner.email(),
				'rate': obj.rate,
				'stars': self.get_album_stars(obj),
				'tracks': self.get_album_tracks(obj),
				'reviews': self.get_album_reviews(obj),
			})
		return result

	def get_album_stars(self, album):
		result = []
		for obj in model.SiteAlbumStar.gql('WHERE album = :1', album).fetch(1000):
			result.append({
				'user': obj.user.email(),
				'added': obj.added.strftime('%Y-%m-%d %H:%M:%S'),
			})
		return result

	def get_album_tracks(self, album):
		result = []
		for obj in model.SiteTrack.gql('WHERE album = :1', album).fetch(1000):
			result.append({
				'id': obj.id,
				'title': obj.title,
				'lyrics': obj.lyrics,
				'number': obj.number,
				'mp3_link': obj.mp3_link,
				'mp3_length': obj.mp3_length,
				'ogg_link': obj.ogg_link,
				'ogg_length': obj.ogg_length,
				'duration': obj.duration,
			})
		return result

	def get_album_reviews(self, album):
		result = []
		for obj in model.SiteAlbumReview.gql('WHERE album = :1', album).fetch(1000):
			result.append({
				'author': obj.author.user.email(),
				'published': obj.published.strftime('%Y-%m-%d %H:%M:%S'),
				'rate_sound': obj.rate_sound,
				'rate_arrangement': obj.rate_arrangement,
				'rate_vocals': obj.rate_vocals,
				'rate_prof': obj.rate_prof,
				'rate_average': obj.rate_average,
				'comment': obj.comment,
			})
		return result
